...
struct AnimalListUseCaseTests {
    var useCase = AnimalListUseCaseImpl()
    ...
    func test_animal_properties_are_decoded_correctly() async throws {
        ...
        guard let lionGameEntity = animals.first(where: { $0.id == "lion" }),
              let lion = lionGameEntity as? AnimalImpl else {
            throw TestError.prerequisiteFailed("Lion not found or could not be cast to Animal")
        }
...
struct AnimalCategoryListUseCaseTests {
    var useCase = AnimalCategoryListUseCaseImpl()
